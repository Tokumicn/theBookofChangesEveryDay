...
// BySolar 通过阳历创建
func bySolar(year, month, day, hour, minute, second int) *Calendar {
...
func BySolar(st time.Time) *Calendar {
	return bySolar(st.Year(), int(st.Month()), st.Day(), st.Hour(), st.Minute(), st.Second())
}
...
func ByLunar(lt time.Time, isLeapMonth bool) *Calendar {
	return byLunar(int64(lt.Year()), int64(lt.Month()), int64(lt.Day()), int64(lt.Hour()), int64(lt.Minute()), int64(lt.Second()), isLeapMonth)
}
...
func (calendar *Calendar) ToString() (string, error) {

	bytes, err := calendar.ToJSON()
	if err != nil {
		return "", err
	}
