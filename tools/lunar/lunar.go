...
func NewLunar(t *time.Time) *Lunar {
	...
	// 将计算后的农历日期重新转换为Time并赋值给Lunar.Time
	lunarTime := time.Date(int(year), time.Month(month), int(day), t.Hour(), t.Minute(), t.Second(), t.Nanosecond(), time.Local)
...
func (lunar *Lunar) Animal() *animal.Animal {
	return animal.NewAnimal(utils.YearOrderMod(lunar.year-3, 12))
}
...
func (lunar *Lunar) MonthAlias() string {
	...
	return pre + common.LunarMonthAlias[lunar.month-1] + "月"
}
...
func (lunar *Lunar) DayAlias() (alias string) {
	...
	case 30:
		alias = "三十"
	default:
		alias = common.DateAlias[(int)(lunar.day/10)] + common.NumberAlias[lunar.day%10]
	}
	return
}
